...
def _fear(
    close: float, *, interpretation: FearInterpretation | None = None
) -> FearMetrics:
    return FearMetrics(
        close=close,
        ...
        ma20=close - 1.0,
        ma60=close - 2.0,
        deviation_from_ma20=close / (close - 1.0) - 1.0,
        deviation_from_ma60=close / (close - 2.0) - 1.0,
...
def test_drawdown_bonus_cannot_exceed_three_times(
    recommend: object, fear_close: float, drawdown: float
) -> None:
...
def test_zero_times_means_no_extra_fear_buy_not_cancel_regular_dca(
    recommend: object, fear_close: float, five: float, twenty: float
) -> None:
    assert callable(recommend)
...
def test_ma_interpretation_does_not_change_multiplier() -> None:
    falling = recommend_spy(
        _fear(30.0, interpretation=FearInterpretation.FEAR_FALLING), _etf()
    )
...
def test_metric_windows_must_be_complete(
    calculator: object, length: int, message: str
) -> None:
...
def test_metric_inputs_must_be_finite_positive(
    calculator: object, bad_value: float
) -> None:
    assert callable(calculator)
