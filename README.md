...
盘前主榜统计最近 30 个完整交易日内“单日涨幅 ≥10%”的出现次数：每个满足条件
的交易日计 1 次，连续 5 天满足就计 5 次。个股按次数降序；并列时按最近一次
发生日期降序、再按代码字母序。事件日复合收益不参与排序，避免把次数榜误读为
涨幅榜。下跌事件继续保留在台账供审计，但不混入上涨主榜。收盘后 16:30 ET
推当天新事件，下一交易日 08:00 ET 只读台账推 30 日次数榜；不足 30 个完整
交易日时拒绝发送旧榜。

`feed` 可选 `hybrid`/`sip`；生产账户
当前无 SIP 权限，采用 IEX 全市场 8% 初筛，再用 Yahoo adjusted 日线确认 ±10%
并计算完整市场口径的 20 日平均成交额。候选确认覆盖必须至少 95%，未确认候选
保留为不可用审计事件且绝不进入主榜。IEX 混合模式本身只覆盖部分市场，因此
记录实际覆盖数量并明确标为 best-effort，不再用全体 active symbol 的 90% 作为
硬门槛；严格 SIP 模式仍执行完整覆盖门控。混合模式可能漏掉 IEX 初筛未达到 8%
的极端异动，升级 SIP 权限后可切换到严格的单源模式。
...
- 文本指令：`状态`（系统概况）/ `持仓`（截图账户+持仓）/ `计划`（活跃执行
  ...
  情报，现场拉取，如 `期权 MU`）/ `异动榜 [30]` / `异动板块 [板块]` /
...
| `extreme_movers_premarket` | 08:00 ET | NYSE 交易日历 | 只读完整事件台账，推 30 日涨超 10% 次数榜 |
